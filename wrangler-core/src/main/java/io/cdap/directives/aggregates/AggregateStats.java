...
public class AggregateStats implements Directive, Lineage {
  ...
  public UsageDefinition define() {
    ...
    builder.define("emit_result", TokenType.TEXT);
  ...
  public void initialize(Arguments args) throws DirectiveParseException {
    ...
    this.emitResult = "true".equalsIgnoreCase(((Text) args.value("emit_result")).value());
  ...
  public List<Row> execute(List<Row> rows, ExecutorContext context) throws DirectiveExecutionException {
      ...
      if (store.get(SIZE_STORE_KEY) == null) {
      ...
      // Process each row and update the transient store
      for (int i = 0; i < rows.size(); i++) {
          ...
          // Emit result only for the last row
